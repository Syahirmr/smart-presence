...
export default function LivenessCamera({ onSuccess, onCancel }: LivenessCameraProps) {
    ...
    const initCamera = async () => {
      ...
      try {
        ...
        if (videoRef.current) {
          ...
          // Explicitly play video stream for cross-browser safety (Chrome, Firefox, Edge)
          await videoRef.current.play().catch((err) => {
            console.warn('Autoplay failed to trigger in LivenessCamera: ', err);
          });
        }
        setStep('HADAP_DEPAN');
        ...
        if (!isMounted) return;
        console.error('Liveness camera init error: ', err);
  ...
  const getOvalClass = () => {
    switch (step) {
      case 'LOADING_MODEL':
        return 'border-slate-700/60 shadow-none';
      case 'SELESAI':
        return 'border-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.4)] transition-all duration-500';
      default:
        return 'border-blue-500 shadow-[0_0_25px_rgba(59,130,246,0.35)] animate-pulse transition-all duration-300';
    }
  };

  const getMessageClass = () => {
    if (message.includes('Gagal') || message.includes('tidak terdeteksi') || message.includes('Tolong') || message.includes('lebih lebar')) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div 
        className={`relative overflow-hidden bg-black border-4 transition-all duration-300 ${getOvalClass()}`} 
      ...
      <div className={`mt-6 px-6 py-3 w-80 rounded-full border text-center text-sm font-semibold shadow-lg transition-all duration-300 ${getMessageClass()}`}>
        {message}
      </div>

      {onCancel && (
        <button onClick={onCancel} className="mt-4 text-rose-400 font-semibold hover:text-rose-300 transition-colors">
          Batalkan Perekaman
        </button>
      )}
    </div>
